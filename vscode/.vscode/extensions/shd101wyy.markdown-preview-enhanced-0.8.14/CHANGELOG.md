...
- Updated [flake.nix](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/blob/HEAD/flake.nix) and node.js to 20.
...
- Supported to import section of markdown by header id:
  ...
  ![](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/raw/HEAD/test.md#header-id)
...
- Fixed jump to header from link like `[link](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/blob/HEAD/test.md#header-id)`.
...
- Added two more syntaxes to import files in addition to the `@import` syntax. Please note that these syntaxes only work on new lines. For example, they won't work within list items.
  - Use the image syntax but with other file extensions:
    ```markdown
    ![](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/raw/HEAD/path/to/file.md)
    ![](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/raw/HEAD/path/to/test.py){.line-numbers}
    ![](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/raw/HEAD/path/to/test.js){code_block=true}
    ```
...
- Deprecated the `processWikiLink` in `parser.js`. Now `crossnote` handles how we process the wiki link.  
  ...
  - `wikiLinkTargetFileExtension`: The file extension of the target file. Default is `md`. For example:
    - `[[test]]` will be transformed to `[test](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/blob/HEAD/test.md)`
    - `[[test.md]]` will be transformed to `[test](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/blob/HEAD/test.md)`
    - `[[test.pdf]]` will be transformed to `[test](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/blob/HEAD/test.pdf)` because it has a file extension.
...
- Supported to set attribute to image and link, e.g.:
  ...
  ```markdown
  ![](https://github.com/shd101wyy/vscode-markdown-preview-enhanced/raw/HEAD/path/to/image.png){width=100 height=100}
  ```
